...
namespace TechCareerWar.Core.Services
{
    public class WeaponStore
    {
        ...
        {
            if (_weapons == null)
                throw new ArgumentNullException(Exceptions.WeaponsNull);

            Weapon weapon = _weapons.ElementAtOrDefault(index);

            if (weapon == null)
                throw new IndexOutOfRangeException(Exceptions.WeaponsIndexOutOfReach);
        ...
        public Weapon BuyRandom()
        {
            ...
            if (_weapons.Count == 0)
                throw new ArgumentException(Exceptions.WeaponsItemCountZero);
